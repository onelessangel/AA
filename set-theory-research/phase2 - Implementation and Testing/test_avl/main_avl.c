...
int main()
{
	...
	// creates the avl tree based on the input type
	if (!strcmp(input_type, "int")) {
		type = 0;
		tree = avl_create((int (*)(const void*, const void*))(compare_ints));
	} else {
		type = 1;
		tree = avl_create((int (*)(const void*, const void*))(compare_strings));
	}
